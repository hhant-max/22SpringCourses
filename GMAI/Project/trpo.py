...
config = {
    ...
    "TRAIN_MODEL_NAME": 'trpo',  # name to use when saving the trained agent.
    "TEST_MODEL_NAME": 'trpo',  # name to use when loading the trained agent.
    ...
    "RECORD_TEST_VIDEOS": True,  # if True, records videos of all episodes done during evaluation.
}
experiment_name = f"trpo_{int(time.time())}"
...
def train():
    ...
    model = TRPO('CnnPolicy', env, verbose=1, tensorboard_log=f"runs/{experiment_name}")
...
def test():
    ...
    model = TRPO.load(config["TEST_MODEL_NAME"], verbose=1)
    model.set_env(env)
    ...
    for episode in range(config["TEST_EPISODES"]):
        ...
        for i in range(config["TREECHOP_STEPS"]):
            action = model.predict(obs.copy(),deterministic=True)
...
def main():
    ...
    sta = time.time()
    train()
    print(f'end with time{(time.time()-sta)/60}')
    #test()
